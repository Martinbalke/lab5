...
function multiply(a, b) { 
    ...
    let numsString = `The product of ${a} and ${b} is ${multiplyNums}.`
    return [multiplyNums, numsString];
...
function sumArray(sumArr) { 
  let numbers1 = sum(sumArr[0], sumArr[1])[0];
  let numSum = sum(numbers1, sumArr[2])[0];
...
function multiplyAnyArray(dynamicArray) {
    ...
    for (var i = 0; i < dynamicArray.length; i++){
        let numMultiply = multiply(number, dynamicArray[i])[0];
        number = numMultiply; 
    }
    let multiplyString = `The numbers ${dynamicArray} have a product of ${number}.`
  return [number, multiplyString];
}
